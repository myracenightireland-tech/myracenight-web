...
class ApiClient {
  ...
  private async request<T>(
    ...
    // Check if this is an auth endpoint (login, register, refresh) - don't try token refresh for these
    const isAuthEndpoint = endpoint.startsWith('/auth/');

    // Handle 401 Unauthorized - try token refresh (but NOT for auth endpoints)
  ...
  async openBetting(raceId: string): Promise<Race> {
    return this.request<Race>(`/races/${raceId}/open-betting`, {
      method: 'POST',
    });
  }
