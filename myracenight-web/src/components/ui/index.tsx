...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = 'primary', size = 'md', isLoading, leftIcon, rightIcon, children, disabled, ...props }, ref) => {
    const baseStyles = 'inline-flex items-center justify-center font-semibold rounded-lg transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-night disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98]';
      ...
      <div className="w-full">
        ...
        <div className="relative">
          ...
          <input
            ref={ref}
            className={clsx(
              'w-full py-3 bg-night-lighter border rounded-lg text-white placeholder-gray-500 transition-all duration-200',
...
export function StatCard({ title, value, icon, subtitle, change, className }: StatCardProps) {
  return (
    <Card className={clsx('relative overflow-hidden', className)} padding="sm">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="text-xs lg:text-sm text-gray-400 mb-1 truncate">{title}</p>
          <p className="text-xl lg:text-3xl font-bold font-display gradient-text">{value}</p>
          {subtitle && (
            <p className="text-xs lg:text-sm text-gray-400 mt-1 truncate">{subtitle}</p>
          )}
          {change && (
            <p className={clsx('text-xs lg:text-sm mt-2', change.positive ? 'text-green-400' : 'text-red-400')}>
              {change.positive ? '↑' : '↓'} {Math.abs(change.value)}%
            </p>
          )}
        </div>
        <div className="p-2 lg:p-3 bg-gold/10 rounded-lg text-gold flex-shrink-0">
          {icon}
        </div>
      </div>
