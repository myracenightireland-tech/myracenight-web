...
export default function RaceManagementPage() {
  ...
  // Get detailed race status based on state
  const getRaceDisplayStatus = (race: Race) => {
    const expectedHorses = 8; // TODO: Get from metadata
    const assignedHorses = horses?.filter(h => h.raceId === race.id).length || 0;
    const hasCommentary = commentaryStatus[race.id]?.hasCommentary;

    // If race has a final status, use it
    ...
    // PENDING status - show detailed pre-race status
    if (assignedHorses === 0) {
      return { label: 'No Horses Assigned', variant: 'error' as const, detail: `0/${expectedHorses} slots filled` };
    }
    if (assignedHorses < expectedHorses) {
      return { label: 'Slots Incomplete', variant: 'warning' as const, detail: `${assignedHorses}/${expectedHorses} slots filled` };
    }
    if (!hasCommentary) {
      return { label: 'Ready for Commentary', variant: 'default' as const, detail: 'Generate commentary' };
    }
  ...
  const getCommentaryBadge = (raceId: string) => {
    ...
    if (!status) return <Badge variant="default"><Clock className="w-3 h-3 mr-1" />Pending</Badge>;
