...
export default function RacesSponsorsPage() {
  ...
  // State for autofill (v41)
  const [autofillingRace, setAutofillingRace] = useState<Record<string, boolean>>({});
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.log('Loading races for event:', currentEvent.id);
      const [racesData, horsesData] = await Promise.all([
        api.getEventRaces(currentEvent.id),
        api.getEventHorses(currentEvent.id),
      ]);
      console.log('Loaded races:', racesData?.length || 0, 'horses:', horsesData?.length || 0);
      setRaces(racesData || []);
      ...
      console.error('Error loading races/horses:', err);
      setError(err.message || 'Failed to load races. Please try refreshing the page.');
      setRaces([]);
      setHorses([]);
    } finally {
      setIsLoading(false);
  ...
  // Autofill a single race (v41)
  const handleAutofillRace = async (raceId: string) => {
    const race = races.find(r => r.id === raceId);
    if (!race) return;
    
    const requiredHorseCount = race.requiredHorseCount || 8;
    const horsesInRace = horses.filter(h => h.raceId === raceId).length;
    
    if (horsesInRace >= requiredHorseCount) {
      // Already full
      return;
    }
    
    setAutofillingRace(prev => ({ ...prev, [raceId]: true }));
    setError('');
    
    try {
      await api.autoFillRace(raceId, requiredHorseCount);
      await loadData();
      await refreshEvent();
    } catch (err: any) {
      setError(err.message || 'Failed to autofill race');
    } finally {
      setAutofillingRace(prev => ({ ...prev, [raceId]: false }));
    }
  };

  // Autofill all races (v41)
  const handleAutofillAllRaces = async () => {
    const racesNeedingFill = races.filter(race => {
      const requiredHorseCount = race.requiredHorseCount || 8;
      const horsesInRace = horses.filter(h => h.raceId === race.id).length;
      return horsesInRace < requiredHorseCount;
    });
    
    if (racesNeedingFill.length === 0) {
      return; // All races already full
    }
    
    if (!confirm(`Autofill remaining slots for ${racesNeedingFill.length} race(s)? This will add auto-generated horses to fill empty positions.`)) {
      return;
    }
    ...
    try {
      for (const race of racesNeedingFill) {
        setAutofillingRace(prev => ({ ...prev, [race.id]: true }));
        await api.autoFillRace(race.id, race.requiredHorseCount || 8);
        setAutofillingRace(prev => ({ ...prev, [race.id]: false }));
      }
      await loadData();
      await refreshEvent();
    } catch (err: any) {
      setError(err.message || 'Failed to autofill races');
    } finally {
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-400">
                          ...
                          {' • '}
                          {horsesInRace.length} / {race.requiredHorseCount || 8} horses
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      ) : race.status === 'READY_TO_RACE' || race.status === 'BETTING_OPEN' || race.status === 'BETTING_CLOSED' || race.status === 'IN_PROGRESS' || race.status === 'COMPLETED' ? (
                        <Badge className="bg-green-500 text-white">✅ Ready to Race</Badge>
                      ) : race.status === 'READY_FOR_COMMENTARY' ? (
                        <Badge className="bg-amber-500 text-white">🎙️ Ready for Commentary</Badge>
                      ) : (
                        <Badge className="bg-gray-500 text-white">⚪ Add Horses</Badge>
                      )}
                      {isExpanded ? (
                          ...
                          {horsesInRace.length > 0 && (
                            <div className="mb-6">
                              <h5 className="text-sm font-medium text-gray-300 mb-3">
                                Horses in this race ({horsesInRace.length}/{race.requiredHorseCount || 8})
                              </h5>
                          ...
                          <div className="mb-4 p-3 bg-night rounded-lg">
                            ...
                            <div className="h-1.5 bg-night-lighter rounded-full overflow-hidden mt-2">
                              <div 
                                ...
                                style={{ width: `${(horsesInRace.length / (race.requiredHorseCount || 8)) * 100}%` }}
                              />
                            </div>
                            {horsesInRace.length < (race.requiredHorseCount || 8) && (
                              <p className="text-xs text-gray-500 mt-1">
                                {(race.requiredHorseCount || 8) - horsesInRace.length} positions will be auto-filled
                              </p>
                            )}
                          </div>

                          <div className="flex gap-3 flex-wrap">
                            {/* Autofill Slots Button (v41) - Show when race needs more horses */}
                            {horsesInRace.length < (race.requiredHorseCount || 8) && !commentaryStatus[race.id]?.hasCommentary && (
                              <Button
                                variant="secondary"
                                onClick={() => handleAutofillRace(race.id)}
                                disabled={autofillingRace[race.id]}
                                isLoading={autofillingRace[race.id]}
                                leftIcon={<Plus className="w-4 h-4" />}
                              >
                                Autofill Slots
                              </Button>
              ...
              <div className="p-6">
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto">
                  {races
                    ...
                    .map((race) => {
                      const horsesInRace = horses.filter(h => h.raceId === race.id).length;
                      const raceMaxHorses = race.requiredHorseCount || 8;
                      const isFull = horsesInRace >= raceMaxHorses;
                        ...
                        >
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-sm text-gray-400">
                                {horsesInRace}/{raceMaxHorses} horses
