...
export default function RacesSponsorsPage() {
  ...
  const handleGenerateCommentary = async (raceId: string) => {
    setGeneratingCommentary(prev => ({ ...prev, [raceId]: true }));
    setError('');
    try {
      await api.generateCommentary(raceId);
      ...
      setGeneratingCommentary(prev => ({ ...prev, [raceId]: false }));
    }
  };
                          ...
                          <div className="flex gap-3">
                            ...
                            {commentaryStatus[race.id]?.hasCommentary ? (
                              <Button
                                ...
                                disabled={generatingCommentary[race.id]}
                                isLoading={generatingCommentary[race.id]}
                                leftIcon={<Sparkles className="w-4 h-4" />}
                              >
                              ...
                              <Button
                                onClick={() => handleGenerateCommentary(race.id)}
                                disabled={generatingCommentary[race.id] || horsesInRace.length === 0}
                                isLoading={generatingCommentary[race.id]}
                                leftIcon={<Sparkles className="w-4 h-4" />}
                              >
                                {horsesInRace.length === 0 ? 'Add Horses First' : 'Generate Commentary'}
                              </Button>
                            )}
