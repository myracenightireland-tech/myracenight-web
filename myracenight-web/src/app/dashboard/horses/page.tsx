...
import { 
  ...
  Sparkles, Users, CheckCircle, MessageSquare, Download,
  History, Calendar
} from 'lucide-react';
...
// PreviousHorse uses Omit to redefine the event property with a lighter structure
interface PreviousHorse extends Omit<Horse, 'event'> {
...
type TabType = HorseApprovalStatus | 'ALL' | 'PREVIOUS';
...
export default function HorsesPage() {
  ...
  const [selectedStatus, setSelectedStatus] = useState<TabType>('PENDING');
  ...
  // Previous Horses state
  const [previousHorses, setPreviousHorses] = useState<PreviousHorse[]>([]);
  const [eventsWithHorses, setEventsWithHorses] = useState<EventWithHorses[]>([]);
  const [previousHorsesLoading, setPreviousHorsesLoading] = useState(false);
  const [selectedSource, setSelectedSource] = useState<'all' | 'organiser' | 'club' | string>('all');
  const [importingHorseId, setImportingHorseId] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (currentEvent && selectedStatus === 'PREVIOUS') {
      loadPreviousHorses();
    }
  }, [currentEvent, selectedStatus, selectedSource]);
  ...
  const loadPreviousHorses = async () => {
    if (!currentEvent) return;
    setPreviousHorsesLoading(true);
    try {
      // Load events with horses (for dropdown)
      const eventsData = await api.getEventsWithHorses(currentEvent.id);
      setEventsWithHorses(eventsData.events || []);

      // Load previous horses based on selected source
      let source: 'organiser' | 'club' | 'event' | undefined;
      let sourceEventId: string | undefined;

      if (selectedSource === 'all' || selectedSource === 'organiser') {
        source = 'organiser';
      } else if (selectedSource === 'club') {
        source = 'club';
      } else {
        // Specific event ID selected
        source = 'event';
        sourceEventId = selectedSource;
      }

      const horsesData = await api.getPreviousHorses(currentEvent.id, source, sourceEventId);
      setPreviousHorses(horsesData);
    } catch (err: any) {
      console.error('Failed to load previous horses:', err);
      setError(err.message || 'Failed to load previous horses');
    } finally {
      setPreviousHorsesLoading(false);
    }
  };

  const handleImportHorse = async (horse: PreviousHorse) => {
    if (!currentEvent || horse.alreadyExists) return;
    
    setImportingHorseId(horse.id);
    setError('');
    setImportSuccess(null);
    
    try {
      await api.importHorse(currentEvent.id, horse.id);
      setImportSuccess(`"${horse.name}" imported successfully!`);
      
      // Refresh both lists
      await Promise.all([loadData(), loadPreviousHorses()]);
      
      // Clear success message after 3 seconds
      setTimeout(() => setImportSuccess(null), 3000);
    } catch (err: any) {
      setError(err.message || 'Failed to import horse');
    } finally {
      setImportingHorseId(null);
    }
  };
  ...
  const filteredHorses = horses.filter(horse => {
    if (selectedStatus === 'ALL' || selectedStatus === 'PREVIOUS') return true;
  ...
  // Count importable previous horses
  const importableCount = previousHorses.filter(h => !h.alreadyExists).length;
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="p-4 lg:p-8">
        ...
        {importSuccess && (
          <div className="mb-4 lg:mb-6 p-3 lg:p-4 bg-green-500/10 border border-green-500/20 rounded-lg flex items-center gap-3 text-green-400 text-sm">
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          {[
            ...
            { value: 'PREVIOUS', label: 'Previous Horses', count: importableCount, icon: History },
          ].map((tab) => (
            <button
              key={tab.value}
              onClick={() => setSelectedStatus(tab.value as TabType)}
        ...
        {/* Previous Horses Tab Content */}
        {selectedStatus === 'PREVIOUS' ? (
          <div className="space-y-4">
            {/* Source Filter Dropdown */}
            <Card className="p-4">
              <div className="flex items-center gap-4 flex-wrap">
                <label className="text-sm font-medium text-gray-400">Filter by source:</label>
                <select
                  value={selectedSource}
                  onChange={(e) => setSelectedSource(e.target.value)}
                  className="px-4 py-2 bg-night border border-night-lighter rounded-lg text-white focus:outline-none focus:border-gold"
                >
                  <option value="all">All My Events</option>
                  <option value="organiser">My Events (Organiser)</option>
                  {currentEvent?.clubId && (
                    <option value="club">Club Events</option>
                  )}
                  {eventsWithHorses.length > 0 && (
                    <optgroup label="Specific Events">
                      {eventsWithHorses.map(event => (
                        <option key={event.id} value={event.id}>
                          {event.name} ({event.horseCount} horses)
                        </option>
                      ))}
                    </optgroup>
                  )}
                ...
                {previousHorsesLoading && <Spinner size="sm" />}
              </div>
            </Card>

            {/* Previous Horses List */}
            {previousHorsesLoading ? (
              <Card>
                <div className="flex items-center justify-center h-32">
                  <Spinner size="lg" />
                </div>
              </Card>
            ) : previousHorses.length === 0 ? (
              <Card>
                <EmptyState
                  icon={<History className="w-12 h-12" />}
                  title="No previous horses found"
                  description="No approved horses from previous events are available to import"
                />
              </Card>
            ) : (
              <div className="space-y-4">
                {previousHorses.map((horse) => (
                  <Card 
                    key={horse.id} 
                    className={`hover:border-night-lighter transition-colors ${
                      horse.alreadyExists ? 'opacity-50' : ''
                    }`}
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-start gap-4 flex-1">
                        <div className="w-16 h-16 bg-purple-500/20 rounded-xl flex items-center justify-center text-3xl flex-shrink-0">
                          🐴
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-3 mb-1 flex-wrap">
                            ...
                            {horse.alreadyExists ? (
                              <Badge variant="default">Already in Event</Badge>
                            ) : (
                          ...
                          {horse.event && (
                            <div className="mt-3 flex items-center gap-2 text-sm text-gray-500">
                              ...
                              <span>
                                From: {horse.event.name} • {format(new Date(horse.event.eventDate), 'MMM d, yyyy')}
                              </span>
                            </div>
                          )}
                        </div>
                      ...
                      <div className="flex items-center gap-2 ml-4 flex-shrink-0">
                        <Button
                          variant={horse.alreadyExists ? 'secondary' : 'primary'}
                          size="sm"
                          onClick={() => handleImportHorse(horse)}
                          disabled={horse.alreadyExists || importingHorseId === horse.id}
                          isLoading={importingHorseId === horse.id}
                        >
                          {horse.alreadyExists ? (
                            'Already Exists'
                          ) : (
          ...
          /* Current Event Horses - existing content */
          <>
