          ...
          <div className="flex flex-wrap gap-4">
            {isUpcoming && event.status !== 'CANCELLED' && (
              <>
                ...
                {!isAuthenticated && (
                  <Link href={`/auth/login?redirect=/events/${slug}/my-dashboard`}>
            ...
            {isAuthenticated && (
              <Link href={`/events/${slug}/my-dashboard`}>
                <Button size="lg" variant="secondary" leftIcon={<Trophy className="w-5 h-5" />}>
                  Go to My Dashboard
                </Button>
              </Link>
            )}
            {event.status === 'LIVE' && (
              <Link href={`/events/${slug}/my-dashboard`}>
                <Button size="lg" leftIcon={<Trophy className="w-5 h-5" />}>
                  {isAuthenticated ? 'Watch Races & Bet' : 'Watch Live'}
                </Button>
              </Link>
            )}
