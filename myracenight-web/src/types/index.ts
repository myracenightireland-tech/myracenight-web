...
// ===== Results + bet-slip read surfaces (served by the backend, derived =====
// ===== strictly from RaceResult / settlement output)                    =====
...
export interface RaceResultView {
  ...
  winner: FinishingOrderEntry | null;
...
export interface BetSlipView {
  ...
  raceName: string;
  raceStatus: string;
  /** Present only in the host (all-attendees) view. */
  punter?: { id: string; name: string };
  horseId: string;
  horseName: string;
  ...
  result: {
    finishPosition: number | null; // null with a result = did not finish
    placed: boolean;
